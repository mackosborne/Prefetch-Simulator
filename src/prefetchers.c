...
struct prefetcher *sequential_prefetcher_new(uint32_t prefetch_amount)
{
    ...
    adjacent_prefetcher->data=(int*)malloc(cache_line * N * sizeof(int));
                                    ...
                                    bool is_miss)
{
    ...
    if is_miss == false
    {
        next_line = address + cache_system->associativity;
    }
    // TODO: Return the number of lines that were prefetched.

    // we're just getting the next line
    return 1;
}
...
struct prefetcher *adjacent_prefetcher_new()
{
    ...
    //size of the next line == line size
    adjacent_prefetcher->data=(int*)malloc(cache_line * sizeof(int));
